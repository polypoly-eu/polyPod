...
extension MessagePackValue: Hashable {
    public func hash(into hasher: inout Hasher) {
        ...
        case .bool(let value): hasher.combine(value.hashValue)
        case .int(let value): hasher.combine(value.hashValue)
        case .uint(let value): hasher.combine(value.hashValue)
        case .float(let value): hasher.combine(value.hashValue)
        case .double(let value): hasher.combine(value.hashValue)
        case .string(let string): hasher.combine(string.hashValue)
        case .binary(let data): hasher.combine(data.count)
        case .array(let array): hasher.combine(array.count)
        case .map(let dict): hasher.combine(dict.count)
        case .extended(let type, let data): hasher.combine(31 &* type.hashValue &+ data.count)
        }
    }
}
