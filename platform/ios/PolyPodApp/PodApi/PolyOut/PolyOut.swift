...
class PolyOut: PolyOutProtocol {
    ...
    func featureFilesPath() -> URL {
        ...
        return documentDirectory
            .appendingPathComponent(Self.fsFilesRoot)
    ...
    private func idFromPodUrl(_ url: String) -> String? {
        if !url.lowercased().starts(with: Self.fsPrefix) {
            return nil
        }

        // Previous polyPod builds used mixed case ("polyPod:") as the protocol
        let id = url
            .replacingOccurrences(of: Self.fsPrefix, with: "", options: .caseInsensitive)
            .replacingOccurrences(of: Self.fsFilesRoot, with: "")
    ...
    func readDir(url: String, completionHandler: @escaping ([[String: String]]?, Error?) -> Void) {
        ...
        let idPrefix = "\(Self.fsPrefix)\(Self.fsFilesRoot)/"
    ...
    func importArchive(url: String, destUrl: String? = nil, completionHandler: @escaping (String?) -> Void) {
        ...
        DispatchQueue.global(qos: .background).async {
            do {
                ...
                let newUrl = Self.fsPrefix + Self.fsFilesRoot + "/" + id
                ...
                UserDefaults.standard.set(fileStore, forKey: Self.fsKey)
