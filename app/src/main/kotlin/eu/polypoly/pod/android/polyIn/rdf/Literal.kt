...
data class Literal(val value: String, val dataType: IRI = IRI(STRING_DATATYPE), val language: String? = "") {

    init {
        if (dataType.iri == STRING_DATATYPE && language == null)
                throw IllegalStateException("Language must not be empty for strings")
        if (dataType.iri != STRING_DATATYPE && language != null)
            throw IllegalStateException("Language must not be set for types other than string")
    }
                ...
                .xmap(
                    {
                        ...
                        val termTypeValue = it["termType"] ?: throw IllegalArgumentException("Expected termType")
                        if (termTypeValue.asStringValue().toString() != "Literal")
                            throw IllegalArgumentException("Expected termType to be 'Literal'")
                        val datatypeValue = it["datatype"] ?: throw IllegalArgumentException("Expected datatype element")
                        val datatype = IRI.codec.decode(datatypeValue)
                        val language = if (datatype.iri == STRING_DATATYPE) {
                            val languageValue = it["language"] ?: throw IllegalArgumentException("For strings language is mandatory")
                            languageValue.asStringValue().toString()
                        } else {
                            null
                        }
                        Literal(value.asStringValue().toString(), datatype, language)
                    },
                    {
                        val map = mapOf(
                        ...
                        if (it.dataType.iri == STRING_DATATYPE) {
                            map["language"] = ValueFactory.newString(it.language)
                        }
                        map
                    }
                )
