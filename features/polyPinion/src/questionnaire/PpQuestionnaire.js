...
export default class PpQuestionnaire extends PpQObject {
    ...
    isLastQuestion(a_question) {
        return this.activeQuestions()[this.activeQuestions().length - 1] === a_question;
    }
    ...
    activeQuestionAfter(a_question) {
        ...
        do {
            index = index + 1;
        } while (index < this._questions.length && !this._questions[index].isActive());
    ...
    hasAnsweredQuestions() {
        return this.answeredQuestions() !== null && this.answeredQuestions().length > 0;
    }
