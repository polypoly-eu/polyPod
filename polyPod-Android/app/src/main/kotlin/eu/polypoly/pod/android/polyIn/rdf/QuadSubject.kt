...
sealed class QuadSubject {
    ...
    companion object {
        val codec: Codec<QuadSubject> =
            Codec.choice(
                Codec.Choice(IRISubject::class.java, IRI.codec.xmap({ IRISubject(it) }, { it.subject })),
                Codec.Choice(BlankNodeSubject::class.java, BlankNode.codec.xmap({ BlankNodeSubject(it) }, { it.subject }))
            )
    }
